                         ...
                         MPL_memory_class class)
{
    ...
    if (num_local == 1)
    {
        ...
        if (local_rank == 0) {
            ...
            MPIR_ERR_CHKANDJUMP1(pmi_errno != PMIX_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmix_put", "**pmix_put %d", pmi_errno);

            pmi_errno = PMIx_Commit();
            MPIR_ERR_CHKANDJUMP1(pmi_errno != PMIX_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmix_commit", "**pmix_commit %d", pmi_errno);
        }

        pmi_errno = PMIx_Get(&MPIR_Process.pmix_proc, PMIX_HOSTNAME, NULL, 0, &pvalue);
        MPIR_ERR_CHKANDJUMP1(pmi_errno != PMIX_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmix_get", "**pmix_get %d", pmi_errno);
        ...
        MPIR_ERR_CHKANDJUMP1(pmi_errno != PMIX_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmix_resolve_peers", "**pmix_resolve_peers %d", pmi_errno);

        PMIX_INFO_CREATE(info, 1);
        PMIX_INFO_LOAD(info, PMIX_COLLECT_DATA, &flag, PMIX_BOOL);
        pmi_errno = PMIx_Fence(procs, nprocs, info, 1);
        MPIR_ERR_CHKANDJUMP1(pmi_errno != PMIX_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmix_fence", "**pmix_fence %d", pmi_errno);
        ...
        if (local_rank != 0) {
            ...
            mpi_errno = MPL_shm_hnd_deserialize(memory->hnd, pvalue->data.string, strlen(pvalue->data.string));
            if (mpi_errno) MPIR_ERR_POP(mpi_errno);
            PMIX_VALUE_RELEASE(pvalue);
