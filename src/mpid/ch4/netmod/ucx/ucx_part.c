                           ...
                           MPIR_Comm * comm, MPIR_Request_kind_t kind, MPIR_Request ** req_ptr)
{
    ...
    if (!is_contig) {
        ...
        MPIDI_UCX_PART_REQ(req).ucp_datatype = ucp_dt_make_contig(dt_size);
                             ...
                             MPIDI_av_entry_t * av, MPIR_Request ** request)
{
    ...
    mpi_errno =
        part_req_create(buf, partitions, count, datatype, rank, tag, comm,
    ...
    am_hdr.partitions = partitions;

    MPI_Aint dtype_size;
    MPIR_Datatype_get_size_macro(datatype, dtype_size);
    am_hdr.data_sz = dtype_size * count * partitions;   /* count is per partition */

    mpi_errno = MPIDI_NM_am_send_hdr(dest, comm, MPIDI_UCX_PART_SEND_INIT, &am_hdr, sizeof(am_hdr));
...
void MPIDI_UCX_precv_matched(MPIR_Request * part_req)
{
    MPI_Aint sdata_size = MPIDI_UCX_PART_REQ(part_req).data_sz;
