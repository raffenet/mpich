                      ...
                      int tag, MPIR_Comm * comm, int context_offset, MPIR_Request ** request)
{
    ...
    *request = sreq;
    ...
    MPIDI_PREQUEST(sreq, context_id) = comm->context_id;
...
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDIG_SEND_INIT);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDIG_SEND_INIT);

    mpi_errno = psend_init(MPIDI_PTYPE_SEND,
                           buf, count, datatype, dest, tag, comm, context_offset, request);
    MPIR_ERR_CHECK(mpi_errno);

  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDIG_SEND_INIT);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
                          ...
                          MPIR_Comm * comm, int context_offset, MPIR_Request ** request)
{
    ...
    mpi_errno = psend_init(MPIDI_PTYPE_SSEND,
                           buf, count, datatype, dest, tag, comm, context_offset, request);
    MPIR_ERR_CHECK(mpi_errno);
                          ...
                          MPIR_Comm * comm, int context_offset, MPIR_Request ** request)
{
    ...
    mpi_errno = psend_init(MPIDI_PTYPE_BSEND,
                           buf, count, datatype, dest, tag, comm, context_offset, request);
    MPIR_ERR_CHECK(mpi_errno);
