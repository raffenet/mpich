                    ...
                    MPIR_Comm * comm, MPIR_Info * info, MPIR_Request ** request)
{
    ...
    int is_local = MPIDI_av_is_local(av);

    mpi_errno =
        MPIDIG_mpi_psend_init(buf, partitions, count, datatype, dest, tag, comm, info, is_local,
                              request);
                    ...
                    MPIR_Comm * comm, MPIR_Info * info, MPIR_Request ** request)
{
    ...
    mpi_errno =
        MPIDIG_mpi_precv_init(buf, partitions, count, datatype, source, tag, comm, info, is_local,
                              request);
    MPIR_ERR_CHECK(mpi_errno);
