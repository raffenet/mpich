                                           ...
                                           MPIR_Request ** req)
{
    ...
    MPIDIG_send_pipeline_cts_msg_t *cts_hdr = am_hdr;
    MPIR_Request *sreq = cts_hdr->sreq_ptr;
...
int MPIDIG_send_pipeline_seg_origin_cb(MPIR_Request * sreq)
{
    ...
    MPIR_Request *parent_req = MPIDIG_REQUEST(sreq, req->plreq).parent_req;
                                           ...
                                           MPIR_Request ** req)
{
    ...
    MPIDIG_send_pipeline_seg_msg_t *seg_hdr = am_hdr;
    MPIR_Request *rreq = (MPIR_Request *) seg_hdr->rreq_ptr;
    MPIR_Assert(rreq);

    int is_done = MPIDIG_recv_copy_seg(data, in_data_sz, rreq);
    if (is_done) {
        MPIDIG_REQUEST(rreq, req->target_cmpl_cb) (rreq);
    }
