...
static void handle_acc_data(MPI_Aint in_data_sz, MPIR_Request * rreq)
{
    ...
    if (MPIDIG_REQUEST(rreq, req->areq.flattened_dt)) {
        MPIR_Datatype *dt = MPIR_Typerep_unflatten(MPIDIG_REQUEST(rreq, req->areq.flattened_dt));
        MPIR_Assert(dt);
...
static int get_target_cmpl_cb(MPIR_Request * rreq)
{
    ...
    MPIR_Datatype *dt = MPIR_Typerep_unflatten(MPIDIG_REQUEST(rreq, req->greq.flattened_dt));
    MPIR_ERR_CHKANDJUMP1(dt == NULL, mpi_errno, MPI_ERR_OTHER, "**nomem", "**nomem %s",
                         "MPIR_Datatype");
                             ...
                             uint32_t attr, MPIR_Request ** req)
{
    ...
    if (msg_hdr->flattened_sz) {
        MPIR_Datatype *dt = MPIR_Typerep_unflatten((char *) am_hdr + sizeof(*msg_hdr));
        MPIR_ERR_CHKANDJUMP1(dt == NULL, mpi_errno, MPI_ERR_OTHER, "**nomem", "**nomem %s",
                             "MPIR_Datatype");
                                  ...
                                  uint32_t attr, MPIR_Request ** req)
{
    ...
    MPIR_Datatype *dt = MPIR_Typerep_unflatten(MPIDIG_REQUEST(rreq, req->preq.flattened_dt));
    MPIR_ERR_CHKANDJUMP1(dt == NULL, mpi_errno, MPI_ERR_OTHER, "**nomem", "**nomem %s",
                         "MPIR_Datatype");
    MPIDIG_REQUEST(rreq, datatype) = dt->handle;
