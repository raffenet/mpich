                                       ...
                                       int requested, int *provided, int *has_args, int *has_env)
{
...
#elif defined(USE_PMIX_API)
    {
        ...
        if (pmi_errno != PMIX_SUCCESS) {
            MPIR_ERR_SETANDJUMP1(pmi_errno, MPI_ERR_OTHER, "**pmix_init", "**pmix_init %d", pmi_errno);
        }
        ...
        if (pmi_errno != PMIX_SUCCESS) {
            MPIR_ERR_SETANDJUMP1(pmi_errno, MPI_ERR_OTHER, "**pmix_get", "**pmix_get %d", pmi_errno);
        }
