...
int MPIR_Init_thread(int *argc, char ***argv, int required, int *provided)
{
    ...
    MPIR_ERR_CHKANDJUMP(MPIR_Process.arch == NULL, mpi_errno, MPI_ERR_INTERN, "**scotcharch");
    ...
    if (MPIR_Process.comm_world->hierarchy_kind == MPIR_COMM_HIERARCHY_KIND__PARENT) {
        SCOTCH_Num sizetab[2], linktab[2] = { 100, 1 };
        int num_nodes;
        ...
        sizetab[0] = num_nodes;
        sizetab[1] = MPIR_Process.comm_world->node_comm ? MPIR_Comm_size(MPIR_Process.comm_world->node_comm) : 1;
        mpi_errno = SCOTCH_archTleaf(MPIR_Process.arch, 2, sizetab, linktab);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);

        /* create reference mapping of ranks to arch nodes */
        MPIR_Process.arch_mapping = MPL_malloc(sizeof(int) * MPIR_Comm_size(MPIR_Process.comm_world), MPL_MEM_OTHER);
