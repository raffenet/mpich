...
int MPIR_TSP_Ibarrier_sched_intra_tsp_auto(MPIR_Comm * comm, MPIR_TSP_sched_t sched)
{
    ...
    switch (MPIR_CVAR_IBARRIER_INTRA_ALGORITHM) {
        ...
        case MPIR_CVAR_IBARRIER_INTRA_ALGORITHM_tsp_dissem:
            mpi_errno =
                MPIR_TSP_Ibarrier_sched_intra_dissem(comm, MPIR_CVAR_IBARRIER_DISSEM_KVAL, sched);
            break;
                ...
                case MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_Ibarrier_intra_tsp_recexch:
                    mpi_errno =
                        MPIR_TSP_Iallreduce_sched_intra_recexch(MPI_IN_PLACE, recvbuf, 0, MPI_BYTE,
                                                                ...
                                                                MPIR_IALLREDUCE_RECEXCH_TYPE_MULTIPLE_BUFFER,
                                                                cnt->u.
                                                                ibarrier.intra_gentran_recexch.k,
                                                                sched);
                    break;

                case MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_Ibarrier_intra_tsp_dissem:
                    mpi_errno =
                        MPIR_TSP_Ibarrier_sched_intra_dissem(comm,
                                                             cnt->u.ibarrier.intra_gentran_dissem.k,
                                                             sched);
                    break;
